...
## 🚧 Roadmap & Known Issues (待办与已知问题)

1. **战斗模拟机制深化**：
   目前计算模型仅基于静态倍率，尚未引入Buff 覆盖率 (Uptime) 及输出手法 (Rotation) 的动态影响。
   不同命座（Mindscape Cinema）改变机制后的伤害分布尚未完善，期待社区大佬协助完善排轴逻辑。

2. **算法与公式验证**：
   现有的“驱动盘严选”算法处于早期阶段，需要更多样本进行边界测试。
   伤害公式基于目前的社区理论数据，可能存在细微偏差，需随游戏版本更新持续校准。

3. **数据库扩容与共建**：
   内置的代理人/音擎/驱动盘数据尚不完整。欢迎提交 PR 补充数据！
   贡献规范建议：添加数据时请注明规格，例如 雅 [0命]、专武 [精1]。
...
*   本项目为粉丝自制工具，计算公式基于社区测绘，可能与实际游戏存在误差。
